...
**spec-kit tasks seed (if available)**

Before dispatching the architect, check if `.shipyard/phases/{N}/SPECKIT-TASKS.md` exists (populated by `/shipyard:import-spec`).

- **If SPECKIT-TASKS.md exists:** Include its full contents in the architect agent prompt with this instruction prepended: "The following tasks were generated by spec-kit's `/speckit.tasks` command. Use them as a starting point — reorganize into Shipyard's wave/plan format, detect dependencies, group into parallel waves where safe, and ensure each plan contains at most 3 tasks. Do not simply copy them verbatim; apply Shipyard's coupling detection and task granularity rules."
- **If SPECKIT-TASKS.md does not exist:** Dispatch the architect normally with no seed input.
